 ...
 * Displays login form and handles user credentials submission
 */
export default function Login(){
  ...
  async function submit(e){
    ...
    try{
      ...
      if(res?.token){ nav('/') }
      else setError('Login failed: ' + JSON.stringify(res))
    }catch(err){ 
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md border border-gray-700 rounded-lg p-8 bg-gray-900">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <button type="submit" className="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors focus-visible:outline-2 focus-visible:outline-green-500">Login</button>
        </form>
        <div className="mt-4 text-center text-sm text-gray-400 space-y-2">
          <div>Don't have an account? <a href="/register" className="text-green-400 hover:text-green-300">Register here</a></div>
          <div>
            <button type="button" onClick={() => { clearAuth(); setEmail(''); setPassword(''); setError(null); }} className="text-xs text-gray-400 hover:text-gray-200 underline">Start fresh (clear local auth)</button>
          </div>
        </div>
      </div>
    </div>
